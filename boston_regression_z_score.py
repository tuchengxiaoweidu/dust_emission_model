...
from sklearn.linear_model import BayesianRidge, LinearRegression, ElasticNet  # 批量导入指标算法
...
for i, pre_y in enumerate(pre_y_list):  # 读出通过回归模型预测得到的索引及结果
    plt.plot(x, ss_y.inverse_transform(pre_y_list[i]), color_list[i], label=model_names[i])  # 画出每条预测结果线
plt.title('regression result comparison data by Z_score')  # 标题
plt.legend(loc='upper right')
plt.xlabel('test data number')
plt.ylabel('real and predicted values')
plt.savefig("z_score regression compare.jpg", dpi=500)
plt.show()

# 模型应用
print('regression prediction:')
new_pre_y = ss_y.inverse_transform(model_gbr.predict(test_data_input))   # 使用GBR进行预测
model_gbr_score = model_gbr.score(test_data_input, test_data_output.ravel())
