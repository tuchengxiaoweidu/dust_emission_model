...
train_x = ss_x.fit_transform(x)  # fit_transform()先拟合数据，再标准化,适合训练数据，测试数据用transform就行
...
BATCH_SIZE = 30
INPUT_SIZE = 13  # sin 数据输入 size ：13个特征
OUTPUT_SIZE = 1  # cos 数据输出 size ：boston房价
...
def get_batch():
    global BATCH_START, TIME_STEPS  # 让函数里面使用的变量是模块层定义的那个全局变量而不是函数内部的局部变量
...
class LSTMRNN(object):
    ...
    def ms_error(self, labels, logits):
        return tf.square(tf.subtract(labels, logits))  # subtract做减法
